...
namespace Microsoft.GS.DPS.Handlers;

public class KeywordExtractingHandler : IPipelineStepHandler
{
    ...
    {
        StepName = stepName;
        _log = (loggerFactory ?? DefaultLogger.Factory).CreateLogger<KeywordExtractingHandler>();
    ...
    {
        _log.LogDebug("Extracting Keywords from the content", pipeline.Index, pipeline.DocumentId);

        foreach (FileDetails uploadedFile in pipeline.Files)
        {
            Dictionary<string, GeneratedFileDetails> extractedTagsFile = new();

            foreach (KeyValuePair<string, GeneratedFileDetails> generatedFile in uploadedFile.GeneratedFiles)
            {
                var file = generatedFile.Value;

                if (file.AlreadyProcessedBy(this))
                {
                    _log.LogDebug("File {FileName} has already been processed by {HandlerName}", file.Name, StepName);
                    continue;
                }

                // Extract keywords from the file
                if (file.ArtifactType == ArtifactTypes.ExtractedText)
                {
                    _log.LogDebug("Extracting Tags from the file {FileName}", file.Name);
                    ...
                    try
                    {
                        ...
                        await _orchestrator.WriteFileAsync(pipeline, destFile, responseBinaryData, cancellationToken).ConfigureAwait(false);
                    ...
                    {
                        _log.LogError(ex, "Error while extracting tags from the file {FileName}", file.Name);
                        await _orchestrator.WriteFileAsync(pipeline, destFile, new BinaryData("[]"), cancellationToken).ConfigureAwait(false);
                        continue;
                    }
                }
            }
